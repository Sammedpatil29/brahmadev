...
const PROD_CDN_URL = 'https://brahmadev-api.democompany.in.net/ota';
...
async function main() {
  ...
  console.log(`\n========================================`);
  console.log(`🚀 Publishing Self-Hosted OTA Bundle v${version}`);
  console.log(`========================================\n`);
  ...
  const bundleFileName = `bundle-${version}.zip`;
  ...
  await new Promise((resolve, reject) => {
    ...
    archive.pipe(output);
    // Include the contents of www (not the 'www' directory itself)
  ...
  const releaseId = `rel-${version}-${Date.now().toString(36)}`;
  ...
  // Also update package.json version in brahmadev
  pkg.version = version;
