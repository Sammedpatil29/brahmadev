...
export class InvoicePage implements OnInit {
  ...
  async buildInvoicePDF(): Promise<{ doc: jsPDF; fileName: string; invoiceId: string }> {
    const doc = new jsPDF({
      orientation: 'p',
      unit: 'mm',
      format: 'a4'
    ...
    const invoiceId = `IN${month}${dateStr}${year}${unixSuffix}`;
    ...
    doc.text('TAX INVOICE', rightEdge, 13, { align: 'right' });
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(brandGold[0], brandGold[1], brandGold[2]);
    doc.text('BILLED TO:', margin + 5, custBoxY + 6);
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(brandGold[0], brandGold[1], brandGold[2]);
    doc.text('INVOICE DETAILS:', midX, custBoxY + 6);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(textMain[0], textMain[1], textMain[2]);
    doc.text('Invoice No: ', midX, custBoxY + 12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(brandNavy[0], brandNavy[1], brandNavy[2]);
    doc.text(invoiceId, midX + 20, custBoxY + 12);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(textMain[0], textMain[1], textMain[2]);
    doc.text(`Invoice Date: ${formattedDate}`, midX, custBoxY + 17.5);
    doc.text('Place of Supply: Karnataka (29)', midX, custBoxY + 23);
    if (hasCustGst) {
      doc.text('Payment Mode: UPI / Bank Transfer', midX, custBoxY + 28.5);
    }
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(brandGold[0], brandGold[1], brandGold[2]);
    doc.text('TERMS & CONDITIONS:', col2X, finalY + 6);
    ...
    doc.text([
      ...
      '2. Official receipt valid only upon realization.',
      '3. Subject to Athani jurisdiction only.',
      '4. Mention invoice ID in transfer reference.'
    ], col2X, finalY + 12);
    ...
    const upiUrl = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(payeeName)}&am=${amount}&tn=${encodeURIComponent(invoiceId)}&cu=INR`;
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setTextColor(textMuted[0], textMuted[1], textMuted[2]);
      doc.text(
        'Notice: Payments are only valid via the bank/UPI details listed on this Invoice. Other payment methods are not authorized or recognized.',
        pageWidth / 2,
        289,
        { align: 'center' }
      );
    ...
    const fileName = `Invoice_${invoiceId}_${cleanCustomer}.pdf`;
    return { doc, fileName, invoiceId };
  }

  saveInvoiceToBackend(doc: jsPDF, invoiceId: string): void {
    try {
      ...
      this.leads.saveInvoice(invoiceData).subscribe({
        next: (res: any) => {
          console.log('Invoice saved to backend successfully', res);
        },
        error: (err: any) => {
          console.warn('Backend save invoice notice:', err);
        }
      });
    } catch (e) {
      console.warn('Error packaging invoice bundle:', e);
    }
  }
  ...
  async previewPDF() {
    ...
    try {
      const { doc, fileName, invoiceId } = await this.buildInvoicePDF();
      this.currentGeneratedDoc = doc;
      this.currentFileName = fileName;
      this.currentInvoiceId = invoiceId;
      this.previewTitle = `Invoice Preview — ${this.customerName || 'Customer'}`;
  ...
  async downloadCurrentPDF() {
    ...
    try {
      ...
      let invoiceId = this.currentInvoiceId;
      if (!doc) {
        const res = await this.buildInvoicePDF();
        doc = res.doc;
      ...
      // Save to backend only on download click
      this.saveInvoiceToBackend(doc, invoiceId);
      ...
      this.isGeneratingPdf = false;
      const toast = await this.toastCtrl.create({
        message: 'Invoice PDF saved and downloaded successfully!',
        duration: 2500,
        position: 'bottom',
        color: 'success'
      });
      ...
      this.isGeneratingPdf = false;
      const toast = await this.toastCtrl.create({
        message: 'Failed to download or share Invoice.',
        duration: 2500,
        position: 'bottom',
        color: 'danger'
      });
  ...
  viewInvoices() { this.navCtrl.navigateForward('/layout/invoice-list'); }
}