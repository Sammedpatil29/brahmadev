...
import { arrowBackOutline, downloadOutline, trashOutline, documentTextOutline, calendarOutline, closeCircleOutline, filterOutline, checkmarkOutline, closeOutline } from 'ionicons/icons';
import { 
  IonHeader, 
  IonContent, 
  IonToolbar, 
...
export class InvoiceListPage implements OnInit {
  ...
  constructor(
    ...
    addIcons({arrowBackOutline,calendarOutline,closeCircleOutline,closeOutline,checkmarkOutline,documentTextOutline,downloadOutline,trashOutline,filterOutline});
  }
  ...
  loadInvoices() {
    this.isLoading = true;
    this.leads.getInvoices().subscribe((res: any) => {
      this.allInvoices = res || [];
      this.handleSearch();
      this.isLoading = false;
    }, error => {
      console.error('Error fetching invoices:', error);
      this.presentToast('Failed to load invoices.');
      this.isLoading = false;
    });
  }
  ...
  async download(item: any) {
    ...
    try {
      const cleanCustomer = (item.customerName || 'Customer').replace(/[^a-zA-Z0-9]/g, '_');
      const cleanInvoiceId = (item.invoiceId || 'Invoice').replace(/[^a-zA-Z0-9]/g, '_');
      const fileName = `${cleanInvoiceId}_${cleanCustomer}.pdf`;

      // 1. Fetch PDF binary from backend download stream or direct URL fallback
      let blob: Blob | null = null;
      const downloadEndpoint = `${this.leads.url}/invoices/${item.id}/download`;
      ...
      if (blob) {
        if (Capacitor.isNativePlatform()) {
          ...
          await this.presentToast('Invoice PDF ready!');
        } else {
      ...
      await this.presentToast('Failed to download invoice PDF. Please try again.');
    } finally {
  ...
  async delete(item: any) {
    const alert = await this.alertController.create({
      ...
      message: `Are you sure you want to delete the invoice for "${item.customerName}"?`,
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel',
        },
        {
          ...
          cssClass: 'danger',
          handler: () => {
            this.leads.deleteInvoice(item.id).subscribe(() => {
              this.loadInvoices();
              this.presentToast('Invoice deleted.');
            }, error => {              
              console.error('Error deleting invoice:', error);
              this.presentToast('Failed to delete invoice.');
            });
          },
        },
      ],
  ...
  handleSearch() {
    ...
    if (query) {
      filtered = filtered.filter((inv: any) =>
        ...
        (inv.siteAddress && inv.siteAddress.toLowerCase().includes(query)) ||
        (inv.contact && inv.contact.toLowerCase().includes(query)) ||
        (inv.email && inv.email.toLowerCase().includes(query)) ||
        (inv.invoiceId && inv.invoiceId.toString().toLowerCase().includes(query)) ||
        (inv.customerGst && inv.customerGst.toLowerCase().includes(query))
      );
    }
